(() => {
  ...
  (() => {
    ...
    if (!buy) return;
    const BASE = 299;
    const img = document.getElementById('sbBuyImg');
    const finishLabel = document.getElementById('sbBuyFinish');
    const careRow = document.getElementById('sbBuyCareRow');
    const totalEl = document.getElementById('sbBuyTotal');
    const monthlyEl = document.getElementById('sbBuyMonthly');
    const addBtn = document.getElementById('sbAddBag');

    const gbp = n => '£' + n.toFixed(2);

    function selectedCare() {
      const el = buy.querySelector('input[name="care"]:checked');
      return el ? parseInt(el.value, 10) || 0 : 0;
    }

    function render() {
      const care = selectedCare();
      const total = BASE + care;
      if (careRow) careRow.hidden = care === 0;
      if (totalEl) totalEl.textContent = gbp(total);
      if (monthlyEl) monthlyEl.innerHTML =
        'or ' + gbp(total / 12) + '/mo. for 12&nbsp;mo. at 0% APR<sup>*</sup>';
    }

    // keep the selected-option outline in sync + react to changes
    buy.querySelectorAll('.sb-buy__options').forEach(group => {
      group.addEventListener('change', () => {
        group.querySelectorAll('.sb-opt').forEach(opt => {
          const input = opt.querySelector('input');
          opt.classList.toggle('is-selected', input && input.checked);
        });

        const finish = buy.querySelector('input[name="finish"]:checked');
        if (finish && img) {
          const src = img.dataset[finish.value];
          if (src) {
            img.style.opacity = '0';
            setTimeout(() => { img.src = src; img.style.opacity = '1'; }, 160);
          }
          if (finishLabel) finishLabel.textContent = finish.value === 'black' ? 'Space Grey' : 'White';
        }
        render();
      });
    });
